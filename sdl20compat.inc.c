...
static SDL_Surface *SDL_SetVideoMode(int width, int height, int bpp,
                                     Uint32 flags) {
  if (!sdl2_window) {
    sdl2_window =
    ...
    if (!sdl2_rendr)
      goto die;
    sdl2_screen_tex =
        SDL_CreateTexture(sdl2_rendr, SDL_PIXELFORMAT_RGBA32,
...
static void SDL_SetPalette(SDL_Surface *surf, int flag, SDL_Color *pal,
                           int begin, int count) {
  (void)surf;
  (void)flag;
  (void)pal;
...
static int SDL_WM_ToggleFullScreen(SDL_Surface *screen) {
  ...
  return SDL_SetWindowFullscreen(
             sdl2_window, fullscreen ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0) == 0;
}
...
// the above function now returns array indexed by scancodes, so we need to use
// those constants
